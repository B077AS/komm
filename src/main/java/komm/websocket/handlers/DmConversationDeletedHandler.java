...
import komm.websocket.messages.payloads.DmConversationDeletedPayload;

public class DmConversationDeletedHandler implements WsInboundMessageHandler {

    private final Gson gson = GsonProvider.get();

    @Override
    public WsMessageType getType() {
        return WsMessageType.DM_CONVERSATION_DELETED;
    }

    @Override
    public void handle(JsonObject payload) {
        DmConversationDeletedPayload p = gson.fromJson(payload, DmConversationDeletedPayload.class);
        Platform.runLater(() -> {
